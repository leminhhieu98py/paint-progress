...
type PersistedCell = MeshCell & { id: string }
...
/**
 * Which stage rows a `saveWorkStages` call would delete, identified by id.
 *
...
/**
 * Writes the coats of one (work, deck) so they match `stages`, keyed by id.
 *
 * Both keys go on every row: a coat with the wrong work_id would land in
 * another discipline and count towards a percentage it has nothing to do with.
 *
 * Identity is the stage's id; `seq` is display order and nothing more. That
 * distinction is the whole point of this function. `cell_states.stage_id` and
 ...
 * cell_states.stage_id and every zones.stage_id keeps pointing at the stage the admin
 ...
 * afterwards. The ids of existing stages come from `listWorkStages`, so they are the
 * database's own.
 *
 ...
 * 0018 carried the deferral onto `unique (deck_id, seq)`, 0024 onto
 * `unique (work_id, deck_id, seq)`.
 *
 ...
 * (0003) and `cell_states.stage_id ... on delete set null` (0001), so deleting the
 ...
 * key value violates unique constraint "deck_stages_work_id_deck_id_seq_key"`:
